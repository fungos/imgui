    ...
    if (first_begin_of_the_frame)
    {
        ...
        else
        {
            ...
            if (!(flags & ImGuiWindowFlags_NoTitleBar) && !window->DockIsActive)
            {
                ...
                if (window->Flags & ImGuiWindowFlags_TitleMenuBar)
                    title_bar_col = GetColorU32(ImGuiCol_TitleBgActive); // should we check OS focus and update this?
                else
        ...
        if (!(flags & ImGuiWindowFlags_NoTitleBar) && !window->DockIsActive)
        {
            ...
            if (p_open != NULL)
            {
                ...
                const float heighted_pad = (flags & ImGuiWindowFlags_TitleMenuBar) ? (window->TitleBarHeight() - g.FontSize) * 0.5f : 0.0f;
                const float pad = ImMax(style.FramePadding.y, heighted_pad);
            ...
            ImVec2 text_size;
            text_size = CalcTextSize(name, NULL, true) + ImVec2(marker_size_x, 0.0f);
            ImRect text_r = title_bar_rect;
            const float button_width = (flags & ImGuiWindowFlags_TitleMenuBar) ? (window->TitleBarHeight() - 2) : g.FontSize;
            float pad_left = (flags & ImGuiWindowFlags_NoCollapse) ? style.FramePadding.x : (style.FramePadding.x + button_width + style.ItemInnerSpacing.x);
            float pad_right = (p_open == NULL)                     ? style.FramePadding.x : (style.FramePadding.x + button_width + style.ItemInnerSpacing.x);
