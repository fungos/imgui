#define IMGUI_INCLUDE_IMGUI_USER_INL 1

#include "../imgui.h"
#include "../imgui_internal.h"
#include "../imgui_mainwindow.h"
...
static ImVec2 CustomRichTextElementDrawCallback(ImVec2 pos, ImDrawList *drawlist, const char* tag_begin, const char *tag_end)
{
	...
	if (memcmp("icon.png", tag_begin, size) == 0)
	{
		...
		ImGui::ImageButton(my_tex_id, ImVec2(19, 19), ImVec2(0, 0), ImVec2(19.0f / my_tex_w, 19 / my_tex_h), 0, ImColor(0, 0, 0, 255));
	...
	else if (memcmp("healthbar", tag_begin, size) == 0)
	{
		...
		ImGui::PlotLines("HP", values, IM_ARRAYSIZE(values), values_offset, "", -1.0f, 1.0f, ImVec2(30, 19));
...
int main(int argc, char**argv)
{
	...
	SDL_Window* window = SDL_CreateWindow("Dear ImGui SDL2+OpenGL3 example", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 800, 480, SDL_WINDOW_OPENGL | SDL_WINDOW_BORDERLESS);
	...
	bool drag = false;
	int posInWindowX = 0, posInWindowY = 0;
	int startDragX = 0, startDragY = 0;
	int firstclick = 0;
	int dragstart = 0;
	...
	auto main_window = ImGui::GetMainViewport();
	...
	SDL_Rect rect;
	rect.x = int(monitor.WorkPos.x + 1);
	rect.y = int(monitor.WorkPos.y + 1);
	rect.w = int(monitor.WorkSize.x - 2);
	rect.h = int(monitor.WorkSize.y - 2);

	int mainId = SDL_GetWindowID(window);
	...
	while (!done)
	{
		...
		while (SDL_PollEvent(&event))
		{
			...
			if (event.type == SDL_WINDOWEVENT && event.window.event == SDL_WINDOWEVENT_CLOSE && event.window.windowID == mainId)
				done = true;
		}
		...
		if (ImGui::BeginTitleMenuBar("{icon.png} App Main Window Custom Title Bar - {healthbar} - ", &close_win))
		{
